...
!
   ...
   10 CONTINUE
      GOTO 90
   50 CONTINUE
!         TYPE*,TN1,TN2,TN3,TGN1,TGN2,TGN3
...
!      ITERATION LOOP
   10 CONTINUE
        ...
        DIFF=PL-ALOG10(P)
!      TYPE*,PRESS,DIFF*2.302,Z,ZI,IYD,GLAT
        IF(ABS(DIFF).LT.TEST) GOTO 20
        XM=D(6)/XN/1.66E-24
        IF(IMR.EQ.1) XM = XM*1.E3
        G=GSURF/(1.+Z/RE)**2
        SH=RGAS*T(2)/(XM*G)
!         NEW GUESS USING CURRENT SCALE HEIGHT
        Z=Z-SH*DIFF*2.302
        GOTO 10
   20 CONTINUE
      ALT=Z
...
!      G2(A)=(A-4.+(P(95)-1.)*(A-4.+(EXP(-ABS(P(94))*(A-4.))-1.)/ABS(P(94
!     *))))
!      G2(A)=G0(A)*(1.+P(94)*A)
!       Eq. A24c
...
!     $ *EX**(SECM/10800.-.5))/SUMEX(EX)
!      SG2(EX)=(G2(AP(2))+(G2(AP(3))*EX+G2(AP(4))*EX*EX+G2(AP(5))*EX**3
!     $ +(G2(AP(6))*EX**4+G2(AP(7))*EX**12)*(1.-EX**8)/(1.-EX))
!     $  *EX**(SECM/10800.-.5))/SUMEX(EX)
      IF(ISW.NE.64999) CALL TSELEC(SV)
      DO 10 J=1,14
       T(J)=0
...
!      EXP3 = EXP(-10800.*ABS(P(144)))
!      IF(EXP3.GT..99999) EXP3=.99999
!      EXP4 = EXP(-10800.*ABS(P(100)))
!      IF(EXP4.GT..99999) EXP4=.99999
!      IF(P(94).LT.1.E-4) P(94)=1.E-4
...
!        UT AND MIXED UT,LONGITUDE
      ...
      ((P(69)*PLG(2,1)+P(70)*PLG(4,1)+P(71)*PLG(6,1))*&
           COS(SR*(SEC-P(72))))
!     $+(P(73)*PLG(2,2)+P(74)*PLG(4,2)+P(75)*PLG(6,2))*SWC(11)*
!     $     COS(SR*SEC-DGTR*LONG)*(1.+P(138)*DFA*SWC(1))
!     $ +(P(87)*PLG(2,2)+P(88)*PLG(4,2)+P(89)*PLG(6,2))*SWC(11)*
!     $      SIN(SR*SEC-DGTR*LONG)*(1.+P(138)*DFA*SWC(1))
      T(12)=T(12)+SWC(11)*&
