...
!*  chapman.for                                                      *C
!*                                                                   *C
!*  Written by:  David L. Huestis, Molecular Physics Laboratory      *C
!*                                                                   *C
!*  Copyright (c) 2000  SRI International                            *C
!*  All Rights Reserved                                              *C
!*                                                                   *C
!*  This software is provided on an as is basis; without any         *C
!*  warranty; without the implied warranty of merchantability or     *C
...
!*
!*	  X = (R+z)/H
!*	  chi0 = solar zenith angle (in degrees)
!*
!*	  implicit real*4(a-h,o-z)
!*	  depth = atm_chapman(X,chi0)	! analytical
!*	  depth = atm_chap_num(X,chi0)	! numerical (chi0 .le. 90)
!*
!*	  implicit real*8(a-h,o-z)
!*	  depth = atm8_chapman(X,chi0)	! analytical
!*	  depth = atm8_chap_num(X,chi0)	! numerical (chi0 .le. 90)
!*
!*  PERFORMANCE:
!*
...
!*
!*	  120	atm_chapman and atm8_chapman for X .lt. 36
!*	   25	atm_chapman and atm8_chapman for X .ge. 36
!*	  500	atm_chap_num
!*	 5000	atm8_chap_num
!*
!*    ACCURACY (maximum relative error, 0.le.chi0.le.90, 1.le.X.le.820)
!*
!*	6.0E-7	atm_chapman and atm8_chapman for X .lt. 60
!*	1.5E-7	atm_chapman and atm8_chapman for X .ge. 60
!*	6.0E-8	atm_chap_num
!*	1.E-15	atm8_chap_num (convergence test)
!*
!*    CODING
!*
...
!*  INTERNET ACCESS:
!*
!*	Source: http://www-mpl.sri.com/software/chapman/chapman.html
!*	Author: mailto:david.huestis@sri.com
!*	        http://www-mpl.sri.com/bios/Huestis-DL.html
!*
!*  EDIT HISTORY:
!*
!*	01/22/2000 DLH	First complete documentation
!*
!*	01/15/2000 DLH	First complete version of chapman.for
!*
!**********************************************************************
!*
!*  THEORY:
...
!*
!*	where zs = (R+z0)*sin(chi0)-R is the tangent height.
!*
!*	    For an exponential atmosphere, with
!*
!*	  n(z) = n(z0) * exp(-(z-z0)/H)
!*
...
!*
!*
!*    CHANGES OF THE VARIABLE OF INTEGRATION
!*
!*	Substituting y = (R+z)/(R+z0) - 1 we find
!*
...
!*	     [ exp(X*(1-sin(chi0)*s)) * s / sqrt(s**2-1) ds ]
!*
!*	From this equation we can establish that
!*
!*	  Ch(X,90) = X*exp(X)*K1(X)
!*
...
!*	eqn (10)].  If we first expand the square root in (B)
!*
...
!*		    + K1(X*sin(chi0))*I0(y*sin(chi0)) ) dy ]
!*
!*	The function atm8_chap_deq(X,chi0) evaluates this expression.
!*	Since explicit approximations are available for I1(z) and K1(z),
!*	the remaining challenge is evaluation of the integrals
!*
...
!*
!*	which are accomplished by term-by-term integration of ascending
!*	and descending power series expansions of I0(z) and K0(z).
!*
!*  REFERENCES:
!*
...
!*		Function for Atmospheric Attenuation," Appl. Opt. _3_,
!*		640 (1964).
!*
...
!*		Differential Equations," (Academic, NY, 1966).
!*
...
!  ####################################################################
!
!	Chapman function calculated by various methods
!
!	  Ch(X,chi0) = atm_chapman(X,chi0)   : real*4 entry
!	  Ch(X,chi0) = atm8_chapman(X,chi0)  : real*8 entry
!
!	Internal service routines - user should not call, except for
!	testing.
!
!	  Ch(X,chi0) = atm8_chap_asy(X,chi0) : asymptotic expansion
!	  Ch(X,chi0) = atm8_chap_deq(X,chi0) : differential equation
!	  Ch(X,chi0) = atm_chap_num(X,chi0)  : real*4 numerical integral
...
!			[ exp(-x*(t**2-t0**2) * (t**2-t0**2)**n dy ] ]
!
!	with t0**2 = 1 - sin(chi0)
!
!  ====================================================================
...
!	this code fragment calculates
!
!	  xi1 = exp(-x*sin(chi0)) * i1(x*sin(chi0))
!
!  --------------------------------------------------------------------
...
!	this code fragment calculates
!
!	  xk1 = x*sin(chi0) * exp(x*sin(chi0)) * k1(x*sin(chi0))
!
!  --------------------------------------------------------------------
...
!	this chapman function routine calculates
!
!	  ch(x,chi0) = atm_chap_num(x,chi0) = numerical integral
!
!  ====================================================================
...
!
!	  i0(z) = sum{n=0,6} [ ai0(n) * z**(2*n) ]
!	  k0(z) = -log(z)*i0(z) + sum{n=0,6} [ ak0(n) * z**(2*n) ]
!
!	for large values of z we expand in reciprocal powers
!
...
!
!	for small values of x*sin(chi0) we break the integral
!	into two parts (with f(z) = i0(z) or k0(z)):
!
!	  int{y=x,infinity} [ exp(-y) * f(y*sin(chi0)) dy ]
!
!	    = int{y=x,x1} [ exp(-y) * f(y*sin(chi0)) dy ]
!	      + int{y=x1,infinity} [ exp(-y) * f(y*sin(chi0)) dy ]
!
!	where x1 = 3.75/sin(chi0) for i0 and 2/sin(chi0) for k0.
!
!	in the range y=x,x1 we integrate the term-by-term using
!
!	  int{z=a,b} [ exp(-z) * z**(2*n) dz ]
!	    = gamma(2*n+1,a) - gamma(2*n+1,b)
!
!	and a similar but more complicated formula for
!
!	  int{z=a,b} [ log(z) * exp(-z) * z**(2*n) dz ]
!
!	in the range y=x1,infinity we use
!
!	  int{z=b,infinity} [ exp(-z) * z**(-n-0.5) dz]
!	    = gamma(-n+0.5,b)
!
!  ####################################################################
...
!	    write(*,1900)i,sum,gg(i)
!1900	format(i5,1p5d14.6)
        end do
...
!	    write(*,1900)i,sum2,bk0(i),qgamma(i)
        end do
...
!  ####################################################################
!
!	the following "pure math" routines return various combinations
!	of bessel functions, powers, and exponentials.
!
!  ####################################################################

!  ====================================================================
!
!	this bessel function math routine returns
!
!	  xi1 = exp(-|z|) * i1(z)
!
...
!
!	the following "pure math" routines return various combinations
!	of the error function, powers, and exponentials.
!
!  ####################################################################

!  ====================================================================
!
!	this error function math routine returns
!
!	  xerfc(x) = exp(x**2)*erfc(x)
!
!	following press et al. [pft86, p164, erfcc.for]
!
!  ====================================================================
...
!
!	the following "pure math" routines return various combinations
!	of exponential integrals, powers, and exponentials.
!
!  ####################################################################

!  ====================================================================
!
!	this exponential math routine evaluates
!
!	  zxe1(x) = x*exp(x) int{y=1,infinity} [ exp(-x*y)/y dy ]
!
!	following abramowitz and stegun [as64, p229;231, equations
!	5.1.11 and 5.1.56]
!
!  ====================================================================
...
!
!	the following "pure math" routines return various combinations
!	of incomplete gamma functions, powers, and exponentials.
!
!  ####################################################################

!  ====================================================================
!
!	this gamma function math routine calculates
!
!	dn(n) = int{t=z,infinity}
!		[ exp( -(t**2-z**2) ) * (t**2-z**2)**n dt ]
!
!  ====================================================================
...
!	this gamma function math routine calculates
!
!	  gf06(n) = g(n,x) * int{y=x,z} [log(y) * exp(-y) * y**(2*n) dy]
!
!	and
!
!	  gg06(n) = g(n,x) * int{y=x,z} [ exp(-y) * y**(2*n) dy ]
!	          = g(n,x) * ( gamma(2*n+1,x) - gamma(2*n+1,z) )
!
!	for n=0,6, with g(n,x) = exp(x) * max(1,x)**(-2*n)
!
!  ====================================================================
...
!	this gamma function math routine calculates
!
!	  gh13(n) = f(n,x) * int{y=z,infinity} [exp(-y) * y**(n-1) dy]
!	          = f(n,x) * gamma(n,z)
!
!	for n=1,13, with f(n,x) = exp(x) * max(1,x)**(-n+1)
!
!  ====================================================================
...
!  ====================================================================
!
!	this gamma function math subroutine calculates
!
!	  qn(x) = x**n * exp(x) * gamma(-n+0.5,x), n=0,8
!	    = x**n * exp(x) * int{y=x,infinity} [exp(-y)*y**(-n-0.5)dy]
!
!	for x .lt. 2 we first calculate
!
!	  q0(x) = sqrt(pi)*exp(x)*erfc(sqrt(x)) = exp(x)*gamma(0.5,x)
!
!	and use upward recursion.  else, we first calculate
!
!	  q8(x) = x**8 * exp(x) * gamma(-7.5,x)
!
!	following press et al. [pft86, pp162-63, gcf.for] and then
!	recur downward.  also see abramowitz and stegun [as64, 6.5].
!
!  ====================================================================
